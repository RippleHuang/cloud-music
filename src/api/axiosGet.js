...
export default {
   ...
   * ?limit=10&order=hot
   *  limit 取出数量，默认是30
   *  order 分别对应最新和最热,可选值为 'new' 和 'hot'
   ...
   *  timestamp 时间戳，使得每次请求的URL不同，除掉了默认的2分钟缓存
   */
   ...
   *  type 搜索类型 默认为 1 即单曲 这里设置默认返回综合
   ...
   *  threadId 如给动态发送评论，则不需要传 id，需要传动态的 threadId
   */
   ...
   *  commentId 回复的评论id (回复评论时必填),删除评论时传入评论的id
   */
